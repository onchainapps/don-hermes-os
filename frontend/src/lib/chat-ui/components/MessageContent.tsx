...
type Segment = CodeBlock | DiffBlock | TextSegment;

// ─── Parse unified diff block into original + modified ─────────────────────────

function parseDiffBlock(diffText: string): { original: string; modified: string } {
  const originalLines: string[] = [];
  const modifiedLines: string[] = [];
  for (const line of diffText.split('\n')) {
    if (line.startsWith('---') || line.startsWith('+++')) continue;
    if (line.startsWith('-')) originalLines.push(line.slice(1));
    else if (line.startsWith('+')) modifiedLines.push(line.slice(1));
    else { originalLines.push(line); modifiedLines.push(line); }
  }
...
async function highlightCode(language: string, code: string): Promise<string> {
  ...
  if (!highlighter.getLoadedLanguages().includes(resolved)) {
    try {
      ...
      // Language not found, use plaintext
      return `<code class="shiki-plain">${escapeHtml(code)}</code>`;
    }
  }
    ...
    <div class="mc-code-block">
      <div class="mc-code-header">
        ...
        <div class="flex gap-2">
          ...
          <button class="mc-copy-btn" onClick={handleCopy}>
            {copied() ? '✓ copied' : '⎘ copy'}
          </button>
        </div>
      </div>
...
function DiffBlockView(props: { code: string }) {
  const [dismissed, setDismissed] = createSignal(false);
  const { original, modified } = parseDiffBlock(props.code);

  if (dismissed()) return null;

  const handleAccept = () => {
    window.dispatchEvent(new CustomEvent('diff-accept', {
      detail: { original, modified, filePath: '' },
    }));
    setDismissed(true);
  };
  ...
  return (
    <div class="mc-root">
      <For each={segments()}>
        {(segment) => (
          <Show
            ...
            fallback={
              <Show when={segment.type === 'diff'}
                fallback={
                  <CodeBlockView
                    language={(segment as CodeBlock).language}
                    code={(segment as CodeBlock).code}
                  />
                }
              >
                <DiffBlockView code={(segment as DiffBlock).code} />
              </Show>
            }
          >
            <div class="mc-markdown" innerHTML={DOMPurify.sanitize(parseMarkdown((segment as TextSegment).text))} />
          </Show>
        )}
      </For>
      ...
      <style>{`
        ...
        .mc-markdown :global(a) {
          color: #00f3ff;
          text-decoration: underline;
          text-underline-offset: 2px;
        }
        .mc-markdown :global(a:hover) {
          color: #00ff9f;
          text-shadow: 0 0 6px rgba(0, 255, 159, 0.4);
        }

        .mc-markdown :global(code) {
        ...
        .mc-markdown :global(ul),
        .mc-markdown :global(ol) {
          padding-left: 22px;
          margin: 6px 0;
        }
        .mc-markdown :global(li) {
          margin: 3px 0;
        }

        .mc-markdown :global(blockquote) {
          border-left: 2px solid rgba(0, 243, 255, 0.3);
          padding-left: 12px;
          margin: 6px 0;
          color: rgba(170, 255, 204, 0.6);
        }

        .mc-markdown :global(table) {
          border-collapse: collapse;
          margin: 8px 0;
          width: 100%;
        }
        .mc-markdown :global(th) {
        ...
        .mc-markdown :global(td) {
          border: 1px solid rgba(0, 255, 159, 0.1);
          padding: 5px 10px;
        }
        .mc-markdown :global(tr:hover td) {
          background: rgba(0, 255, 159, 0.03);
        }

        .mc-markdown :global(hr) {
          border: none;
          border-top: 1px solid rgba(0, 255, 159, 0.12);
          margin: 12px 0;
        }

        .mc-markdown :global(img) {
          max-width: 100%;
          border-radius: 6px;
          border: 1px solid rgba(0, 255, 159, 0.1);
        }
        ...
        .mc-code-body :global(pre) {
          margin: 0;
          background: none !important;
          padding: 0 !important;
        }
        .mc-code-body :global(code) {
          background: none !important;
        ...
        .mc-code-body :global(.shiki-plain) {
          color: #e0ffe8;
          white-space: pre;
        }

        .mc-code-loading {
          color: rgba(170, 255, 204, 0.3);
          font-style: italic;
          font-size: 12px;
        }
        ...
        .mc-cursor {
          ...
          margin-left: 1px;
          animation: mc-blink 0.7s step-end infinite;
